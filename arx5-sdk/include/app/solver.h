...
class Arx5Solver
{
  public:
    ...
    // Inverse kinematics returns ik_status based on <kdl/solveri.hpp>
    // ik_status == 0 means success
  ...
  private:
    static KDL::Frame pose6d_to_kdl(const Eigen::Matrix<double, 6, 1> &pose);
    static Eigen::Matrix<double, 6, 1> kdl_to_pose6d(const KDL::Frame &frame);

    void validate_joint_vector_size(const Eigen::VectorXd &vec, const std::string &name) const;
    bool within_joint_limit(const Eigen::VectorXd &q, double tol = 1e-8) const;
  ...
  private:
    ...
    // FK: full chain to eef_link
    std::shared_ptr<KDL::ChainFkSolverPos_recursive> fk_solver_;

    // IK / ID: active joints only
