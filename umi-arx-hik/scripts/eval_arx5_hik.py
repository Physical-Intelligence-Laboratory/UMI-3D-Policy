"""
Usage:
(umi-arx): python scripts_real/eval_real_hik.py -i data/outputs/2023.10.26/02.25.30_train_diffusion_unet_timm_umi/checkpoints/latest.ckpt -o data_local/cup_test_data
...
def main(
    ...
    hik_server_bin = os.path.join(
        ROOT_DIR, "cpp", "hik_camera", "build_sys", "hik_server"
    )
    hik_config_paths = [
        os.path.join(ROOT_DIR, "config", "left_camera_trigger.yaml")
    ]
    hik_shm_names = ["/hik_cam0"]

    assert os.path.exists(hik_server_bin), hik_server_bin
    assert os.path.exists(hik_config_paths[0]), hik_config_paths[0]    
    ...
    with SharedMemoryManager() as shm_manager:
        with Spacemouse(
            ...
            hik_server_bin=hik_server_bin,
            hik_config_paths=hik_config_paths,
            hik_shm_names=hik_shm_names,
